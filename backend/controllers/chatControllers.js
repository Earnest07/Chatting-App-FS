...
const accessChat = asyncHandler(async (req, res) => {
  const { userId } = req.body; // This userId is the one which will be sent from the frontend
  ...
  var isChat = await Chat.find({
    ...
    $and: [
      ...
      {
        users: { $elemMatch: { $eq: userId } }, // samnewala user id
      },
    ],
  })
...
//   if (users.length < 2) {
//     return res.status(400).send("Require more than 2 users.");
//   }
